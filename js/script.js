...
function showImage(farmId_attrib, serverId_attrib, id_attrib, secret_attrib, title_attrib) {
    ...
    $("#largerImage").append(
        ...
        +'<h2>'
        + title_attrib + '</h2>'
        + '<img class="img-responsive center" alt="Image of ' + title_attrib + '" src=https://farm'
        + farmId_attrib
        + '.staticflickr.com/'
        + serverId_attrib
        + '/'
...
function jsonExifParser(formatExifURL) {
    ...
    $.getJSON(formatExifURL, function (data) {
        ...
        if(error == 0) {
            $(".exifData").append(
                '<thead><tr><th class="text-center">Title</th><th class="text-center">Value</th></tr></thead>'
                +'<tbody>');
